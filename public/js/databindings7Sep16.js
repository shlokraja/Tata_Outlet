...
function getSalesData()
{
  ...
  .done(function (data)
  {
      $("#sales_cash_table").html(' <div class="table-responsive"><table class="table table-hover sales-cash-table"> <tbody><tr> <th></th><th>Food count</th> <th>Others</th> <th>Cash</th> <th>Card</th> </tr> <tr><td class="sidemenu"> Today</td><td>' + data["dispenser_day_count"] + ' </td><td> ' + data["outside_day_count"] + '</td><td> ' + data["day_cash_amount"] + '</td><td> ' + data["day_card_amount"] + '</td> </tr> <tr>  <td class="sidemenu"> Month</td><td> ' + data["dispenser_month_count"] + '</td><td> ' + data["outside_month_count"] + '</td><td> ' + data["month_cash_amount"] + '</td> <td> ' + data["month_card_amount"] + '</td> </tr> </tbody></table></div>');
  })
  .fail(function (jqxhr, textStatus, error)
  {
      var err_msg = textStatus + ", " + jqxhr.responseText;
