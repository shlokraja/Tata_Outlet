...
function handleIncomingPO(data) {
    ...
    for (var po_id in result_pos)
    {
        ...
        str_incoming_data += '<td>' + current_po["po_id"] + '</td><td>' + date_obj.toDateString() + ' | ' + date_obj.toLocaleTimeString() + '</td>';
        str_incoming_data += '<td>' + total_po_items + '</td><td>' + total_po_quantity + '</td>';
        str_incoming_data += '<td>' + current_po["rest_name"] + '</td>';
        str_incoming_data += "<td> <button data-rest_name='" + current_po["rest_name"] + "' id=" + current_po["po_id"] + "_reconcile class='incoming_po_reconcile btn btn-raised btn-default' onclick='Reconcile(this," + current_po["po_id"] + "," + total_po_items + "," + total_po_quantity + ")' style=padding:5px>Reconcile</button></td>";
