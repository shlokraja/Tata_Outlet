...
module.exports.InitAutomaticReconcile = function () {
    if (!pflag)
    {
        ...
        GetPODetails(false, function (err, res) {
            if (err)
            {
                //callback(err, null);
                console.log("InitAutomaticReconcile: " + err);
                return;
            }
            else
            {
                //callback(null, res);
                console.log("InitAutomaticReconcile success");                
            }
        });
    }
}
...
function GetPODetails(is_end_of_day, callback) {
    var reconcile_items = [];
    var excess_quantity_items = [];

    console.log("GetPODetails function called" + outlet_url);
    ...
    redisClient.get(helper.reconcile_stock_count_node,
        function (err, reply_reconcile_stock_count) {
            ...
            else
            {
                ...
                console.log("reconcile_stock_count: " + JSON.stringify(reply_reconcile_stock_count));
...
function save_reconcile_data_main(reconcile_items, excess_quantity_items, callback) {
    ...
    if (reconcile_items != undefined && reconcile_items.length > 0)
    {
        request({
            ...
            function (error, response, data) {
                ...
                send_restatrant_excess_po_mail_main(reconcile_items, excess_quantity_items, callback);
            });
    }
...
function send_restatrant_excess_po_mail_main(reconcile_items, excess_quantity_items, callback) {
    ...
    if (excess_quantity_items != undefined && excess_quantity_items.length > 0)
    {
        ...
        if (items != undefined && items.length > 0)
        {
            ...
            for (var po_id in restaurant_group_items)
            {
                ...
                if (total_excess_qty > 0)
                {
                    ...
                    excess_mail_content += 'Hi,<br/> Please find the following details of Excess quantity against the PO(' + po_id + ') to <b>' + outlet_config.name + ' </b>  from your Restaurant. <br/><br/><br/><table class="reconsile" border="1" cellpadding="0" cellspacing="0" width="75%">';
...
function send_restautant_excess_mail(restaurant_excess_mails, callback) {
    if (restaurant_excess_mails != undefined && restaurant_excess_mails.length > 0)
    {
        request({
